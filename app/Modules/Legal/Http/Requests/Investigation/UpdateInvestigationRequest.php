...
class UpdateInvestigationRequest extends FormRequest
{
    ...
    public function rules()
    {
        return [
            ...
            'investigation_questions.*'                => 'required|array',
            'investigation_questions.*.id'             => '',
            'investigation_questions.*.question'       => 'required|string|min:2|max:100|regex:/^[A-Za-z0-9()\]\s\[#%&*_=~{}^:`.,$!@+\/-]+$/',
            'investigation_questions.*.answer'         => 'required|string|min:2|max:100|regex:/^[A-Za-z0-9()\]\s\[#%&*_=~{}^:`.,$!@+\/-]+$/',
